...
export default function SubmitPayment({ cardHolderName }: { cardHolderName: React.RefObject<HTMLInputElement> }) {
  ...
  async function onApproveCallback(data: any, actions?: any) {
    try {
      const response = await paypalApiRequest.captureOrder(data.orderID)
